...
if [ ! -d "/root/confwizwiz-2" ]; then

    sudo mkdir /root/confwizwiz-2
    
    sleep 1
    
    touch /root/confwizwiz-2/dbrootwizwiz.txt
    sudo chmod -R 777 /root/confwizwiz-2/dbrootwizwiz.txt
    sleep 1
    ...
    echo "${ASAS}user = 'root';" >> /root/confwizwiz-2/dbrootwizwiz.txt
    echo "${ASAS}pass = '${randomdbpasstxt}';" >> /root/confwizwiz-2/dbrootwizwiz.txt
    echo "${ASAS}path = '${RANDOM_NUMBER}';" >> /root/confwizwiz-2/dbrootwizwiz.txt
    
    sleep 1

    passs=$(cat /root/confwizwiz-2/dbrootwizwiz.txt | grep '$pass' | cut -d"'" -f2)
    userrr=$(cat /root/confwizwiz-2/dbrootwizwiz.txt | grep '$user' | cut -d"'" -f2)
...
# update cron
PATHS=$(cat /root/confwizwiz-2/dbrootwizwiz.txt | grep '$path' | cut -d"'" -f2)
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/wizwizxui-timebot-2/settings/messagewizwiz.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/wizwizxui-timebot-2/settings/rewardReport.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/wizwizxui-timebot-2/settings/warnusers.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "* * * * * curl https://${DOMAIN_NAME}/wizwizxui-timebot-2/settings/gift2all.php >/dev/null 2>&1") | sort - | uniq - | crontab -
(crontab -l ; echo "*/3 * * * * curl https://${DOMAIN_NAME}/wizwizxui-timebot-2/settings/tronChecker.php >/dev/null 2>&1") | sort - | uniq - | crontab -
...
ROOT_PASSWORD=$(cat /root/confwizwiz-2/dbrootwizwiz.txt | grep '$pass' | cut -d"'" -f2)
ROOT_USER="root"
...
wait
    ...
    if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'wizwiz2'") ]]; then
        clear
        ...
        file_path="/var/www/html/wizwizxui-timebot-2/baseInfo.php"
        ...
        # print file
        echo -e "<?php" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "error_reporting(0);" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}botToken = '${YOUR_BOT_TOKEN}';" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}dbUserName = '${dbuser}';" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}dbPassword = '${dbpass}';" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}dbName = '${dbname}';" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}botUrl = 'https://${YOUR_DOMAIN}/wizwizxui-timebot-2/';" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "${ASAS}admin = ${YOUR_CHAT_ID};" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php
        echo -e "?>" >> /var/www/html/wizwizxui-timebot-2/baseInfo.php

        sleep 1

        curl -F "url=https://${YOUR_DOMAIN}/wizwizxui-timebot-2/bot.php" "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook"
        ...
        url="https://${YOUR_DOMAIN}/wizwizxui-timebot-2/createDB.php"
        curl $url
